...
const ButtonGoogle = ({ text = "", onClick = () => {} }) => {
  return (
    <button
      className="flex items-center justify-center w-full px-3 py-3 mb-5 border gap-x-3 border-strock lg:mb-3 dark:border-darkStroke rounded-xl min-h-[56px]"
      type="button"
      onClick={onClick}
    >
      <span>
      ...
      <h3 className="text-base text-text2 dark:text-white">{text}</h3>
    </button>
  );
};
